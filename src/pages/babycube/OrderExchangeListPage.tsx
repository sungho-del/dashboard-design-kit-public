...
} from "./OrderExchangeListPage.data";
import { GNB_LOGO_SLOTS, GNB_SECTIONS } from "../gnbSections";

/* =========================================================================
 * S11 교환 목록 — 뼈대 (BabyCube 본사 운영 어드민)
 *
 * ## 화면 유형: 목록형
 * 교환 요청이 걸린 **처리단위**를 조회한다.
 *
 * ⚠️ 원본 대조에서 밝혀진 사실: **교환 목록의 표도 주문 목록의 표와 같다.**
 * 컬럼 24개가 라벨·순서까지 동일하고 첫 열의 데이터만 교환 상태로 갈린다.
 *
 * ## 이 파일은 도메인을 모른다 — 도메인은 `./OrderExchangeListPage.data.ts` 에 있다
 *
 * | 갈아끼울 것           | 위치                                           |
 * | --------------------- | ---------------------------------------------- |
 * | 데이터·타입·라벨·단위 | `OrderExchangeListPage.data.ts` **전체**       |
 * | 표 컬럼 구성          | 이 파일의 `<colgroup>` + `TableHead` + 본문 셀 |
 * | 조회 조건 구성        | "조회 조건" 카드                                |
 * | 화면 제목·도움말      | `PageHeader` 의 `title` · `badges`             |
 *
 * ## 형제 화면과 같은 뼈대를 **복사해서** 쓴다 (공용화하지 않는다)
 * S08·S09·S10 과 조회 조건 구성이 거의 같지만 각자 독립 파일이다.
 * 페이지는 템플릿이라, 공용 컴포넌트로 묶으면 서비스를 갈아끼울 때
 * **뼈대를 고쳐야** 한다. `.data.ts` 에 같은 도메인 상수가 반복되는 것도 의도된 것이다.
 *
 * ## ⚠️ 이 화면만 다른 두 가지 — 형제 화면을 보고 채우지 말 것
 * 1. **유형(렌트/판매) 필터가 없다.** 형제 셋에는 유형 대시(`chip.dash`)가 있지만
 *    원본 교환 목록에는 `chip` 자체가 없다. 유형 **열**은 그대로 있다 — 필터만 없다
 * 2. **`stat` 파라미터가 유형이 아니라 상태다.** 형제 셋에서 상태는 `flow` 인데
 *    교환은 `flow` 를 아예 읽지 않는다(`.data.ts` 의 `filtersFromQuery` 참고)
 *
 * ## ⚠️ 원본에 없어서 걷어낸 것 — 되살리지 말 것
 * - **원본에 없는 13열**: 검수 판정 · 교환 신청 일시 · 교환 사유 · 재배송 택배사 ·
 *   재배송 송장번호 · 재배송 일시 · 교환 상품코드 · 교환 상품명 · 추가 결제금액 ·
 *   결제금액 · 셀러 · 처리단위 · 주문일시
 * - **증감 요약 카드 3장**(`STATS`). 원본에 없는 지어낸 숫자였다.
 *   ⚠️ 형제 화면과 달리 **그 자리를 유형 대시로 채우지도 않는다** — 원본에 없다
 * - **`판매자`(본사/셀러 배지) + `셀러`(이름) 두 열**. 원본 `b()` 는
 *   `"자체" === owner ? "본사" : sellerName` 으로 **열 하나**로 끝낸다
 * - **행 액션(`교환 승인` 버튼 + 확인 모달)**. 원본 교환 목록에는 행 액션이 없다
 * - **유형 `SegmentedControl`**. 원본에 유형 축이 없다
 * ====================================================================== */
...
export interface OrderExchangeListPageProps {
  navOpen: boolean;
...
export function OrderExchangeListPage({
  navOpen,
  onNavOpenChange,
  activeNav,
  onNavSelect,
}: OrderExchangeListPageProps) {
  const { toast } = useToast();

  /*
   * 원본 교환 목록도 주문 목록과 **같은 셸**을 써서 `stat`·`owner`·`sellerName`·
   * `q`·`paidAt_from/to` 를 읽는다. 받는 쪽이 없으면 **링크는 열리고 필터만 안 걸린다** —
   * 화면은 멀쩡해 보여서 타입·린트로는 안 잡힌다.
   * ⚠️ 여기서 `stat` 은 **상태**다. 형제 화면(유형)과 뜻이 다르다.
   */
  ...
  const [preview, setPreview] = useState<Exchange | null>(null);

  /** 조건 하나를 갈아끼운다. **조건이 바뀌면 페이지는 언제나 1로 되돌아간다** */
  const patch = (next: Partial<ExchangeFilters>) => {
  ...
  return (
      ...
      header={
        <PageHeader
          ...
          badges={
            <Tooltip
              title="교환 상태 흐름"
              content="교환 신청 → 교환 승인 → 수거중 → 수거완료 → 교환 검수 → 교환 재배송 → 교환 완료 순으로 진행됩니다. 교환 반려는 접수 단계의 거절, 교환 거부는 검수 뒤의 거절이며 두 경우 모두 재반송으로 이어집니다."
            >
              <IconButton
                size="small"
                label="교환 상태 도움말"
    ...
    >
      {/*
        ...
        ⚠️ 유형 축이 없다. 형제 화면 셋과 달리 이 화면 위에는 대시 카드도 없다.
      */}
      <Card>
        <CardHeader
          title="조회 조건"
        ...
        <CardBody>
          ...
          <div className="flex flex-wrap items-start gap-2">
            <FormField
              label="상태"
              className="flex-1 min-w-0"
              description="교환 흐름은 렌트·판매가 같습니다."
            >
              <Select
            ...
            {paged.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {/* 유형은 필터 축이 아니지만 **열로는 남는다** — 원본이 그렇다 */}
      ...
      {/*
        교환 미리보기 — 4항목짜리 빠른 확인용이다.
        제목을 "교환 상세"로 두지 않는다. 24열짜리 표의 전체 정보는 모달에 들어가지 않는다.
        ⚠️ 푸터(도메인 액션)를 두지 않는다 — 원본 교환 목록에는 행 액션이 없다.
      */}
      <Modal open={preview !== null} onClose={() => setPreview(null)}>
        <ModalHeader title="교환 미리보기" description={preview?.id} />
        <ModalBody>
          <InfoList>
