...
- **기존 코드 재사용**: 새로 만들기 전에 `src/components/ui/`, `tokens/*.json`, `src/lib/` 등 기존 리소스를 먼저 탐색
...
> 컴포넌트 구현 건은 이 표를 손으로 훑는 대신 `@agent-design-qa`(8항목 자동 검사)에 위임해도 된다.
> 표의 "자동" 항목은 훅이 이미 잡아주므로, **수동** 항목만 직접 확인하면 된다.

| #   | 확인 항목              | 필수 | 구체적 검증 방법                                                                 |
| --- | ---------------------- | ---- | -------------------------------------------------------------------------------- |
| 1   | 타입·린트 통과         | ✅   | 수동 — `npm run typecheck` · `npm run lint`                                      |
| 2   | 테스트 통과            | ✅   | 수동 — `npm test -- --run`                                                       |
| 3   | 빌드 에러 없음         | ✅   | 수동 — `npm run build` 통과                                                      |
| 4   | 헥스/rgb/hsl 미사용    | ✅   | 자동 — `check-design-tokens.mjs`가 저장 시 감지 (`.ts/.tsx/.js/.jsx`만)          |
| 5   | Tailwind 기본값 미사용 | ✅   | 수동 — `bg-red-`, `text-gray-`, `text-sm` 등 기본 클래스 없는지 확인 (훅 미감지) |
| 6   | 토큰 외 spacing 미사용 | ✅   | 수동 — `8px`, `15px` 등 `--spacing-*`에 없는 값 없는지 확인 (훅 미감지)          |
| 7   | story 파일 존재        | ✅   | 자동 — `check-story-exists.mjs`                                                  |
| 8   | 기존 컴포넌트 재사용   | ✅   | 수동 — `src/components/ui/`의 기존 컴포넌트로 대체 가능한지 확인 (현재 37종)     |
| 9   | 토큰 추가 시 빌드 확인 | ✅   | 수동 — `tokens/*.json` 수정 후 `npm run build:tokens` 실행했는지 확인            |
| 10  | 기존 기능 정상 동작    | ✅   | 수동 — 기존 컴포넌트/페이지 깨지지 않았는지 확인                                 |
