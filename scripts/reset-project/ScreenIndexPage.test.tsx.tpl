 ...
 *
 * 함께 못박는 것:
 *   1. 데이터에 있는 묶음이 서고, 카드 수가 데이터와 맞는가
 *   2. 카드 버튼의 접근가능 이름이 **유일한가** — `"열기"` 로만 두면 전부 같아진다
...
describe("ScreenIndexPage (화면 목록)", () => {
  describe("구성", () => {
    ...
    /**
     * ⚠️ 화면이 하나도 없는 묶음은 그리지 않는다. 안 그러면 `reset:project` 직후처럼
     * 템플릿만 남았을 때 **설명만 있고 카드가 없는 빈 섹션**이 세 개 뜬다.
     */
    it("비어 있는 묶음은 그리지 않는다", () => {
      renderPage();

      const shown = new Set(SCREENS.map((entry) => entry.origin));
      for (const empty of ["generated", "classon", "babycube"] as const) {
        if (shown.has(empty)) continue;
        expect(
          screen.queryByRole("region", { name: /생성물/ }),
  ...
  describe("링크", () => {
    it("카드를 누르면 그 화면의 경로가 넘어간다", async () => {
      ...
      await user.click(
        screen.getByRole("button", { name: `템플릿 · ${target.name} 열기` }),
      );
  ...
  describe("접근성", () => {
    ...
    it("묶음마다 이름표가 붙어 있다", () => {
      ...
      const template = SCREENS.filter((entry) => entry.origin === "template");
      const section = screen.getByRole("region", { name: "템플릿" });
      expect(
